 ...
 * scene. Close to this task is the task of AI. If a player's character is attacking a mob, his ATTACK may be interrupted
 ...
 * character may be like "ATTACK", "random walk" and so on. To reach the goal implementation will split it into several
 * small actions, several steps (possibly repeatable). Like "run to target" then "hit it", then if target is not dead -
 * repeat. This flow of simpler steps may be interrupted by incoming events. Like a character's movement was disabled
 ...
 * Behavior, and if client's state for the character is "moving" we send messages to clients to stop the avatar/mob.
 */
