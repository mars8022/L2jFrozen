...
public class MultiSellChoose extends L2GameClientPacket
{
	...
	private int _listId, _entryId, _amount, _enchantment, _transactionTax; // local handling of taxation
	...
	public void runImpl()
	{
		...
		for (MultiSellEntry entry : list.getEntries())
		{
			if (entry.getEntryId() == _entryId)
			{
				...
				// Dont change multisell on exchange to avoid new window open need
				//player.setMultiSellId(-1);
				return;
			}
		}
	...
	private void doExchange(L2PcInstance player, MultiSellEntry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantment)
	{
		...
		// given the template entry and information about maintaining enchantment and applying taxes re-create the instance of the entry
		// that will be used for this exchange i.e. change the enchantment level of select ingredient/products and adena amount appropriately.
		L2NpcInstance merchant = player.getTarget() instanceof L2NpcInstance ? (L2NpcInstance) player.getTarget() : null;
								...
								if (itemToTake.getEnchantLevel() > 0)
								{
									for (L2ItemInstance inventoryContent : inventoryContents)
									{
										if (inventoryContent.getEnchantLevel() < itemToTake.getEnchantLevel())
										{
											...
											if(itemToTake.getEnchantLevel() == 0)
												break; // nothing will have enchantment less than 0. If a zero-enchanted item is found, just take it
										}
									}
								}
		...
		for (MultiSellIngredient e : entry.getProducts())
		{
			...
			else
			{
				...
				{
					sm = new SystemMessage(SystemMessageId.ACQUIRED).addNumber(e.getEnchantmentLevel());
