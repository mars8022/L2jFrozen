...
import com.l2scoria.gameserver.datatables.sql.MaxCheatersTable;
...
import com.l2scoria.gameserver.model.L2MaxPolyModel;
...
import com.l2scoria.gameserver.model.actor.status.NpcStatus;
import com.l2scoria.gameserver.model.entity.event.Lottery;
import com.l2scoria.gameserver.model.entity.event.L2Event;
import com.l2scoria.gameserver.model.entity.event.TvT;
import com.l2scoria.gameserver.model.entity.event.VIP;
...
import com.l2scoria.gameserver.model.scripts.L2RBManager;
...
import com.l2scoria.gameserver.powerpak.Buffer.L2BufferInstance;
 ...
 * a template to fetch some static values. The templates are hardcoded in the client, so we can rely on them.<BR>
 * <BR>
 * L2Character :<BR>
 * <BR>
...
public class L2NpcInstance extends L2Character
{
	...
	/**
	 * Send a packet SocialAction to all L2PcInstance in the _KnownPlayers of the L2NpcInstance and create a new
	 * RandomAnimation Task.<BR>
	 * <BR>
	 */
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Call the L2Character constructor to set the _template of the L2Character (copy skills from template to object
	 * and link _calculators to NPC_STD_CALCULATOR)</li> <li>Set the name of the L2Character</li> <li>Create a
	...
	/**
	 * Send a packet NpcInfo with state of abnormal effect to all L2PcInstance in the _KnownPlayers of the
	 * L2NpcInstance.<BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <B><U> Values </U> :</B><BR>
	 * <BR>
	 * <li>object is a L2FolkInstance : 0 (don't remember it)</li> <li>object is a L2Character : 0 (don't remember it)</li>
	 * <li>object is a L2PlayableInstance : 1500</li> <li>others : 500</li><BR>
	 * <BR>
	 ...
	 * <B><U> Values </U> :</B><BR>
	 * <BR>
	 * <li>object is not a L2Character : 0 (don't remember it)</li> <li>object is a L2FolkInstance : 0 (don't remember
	 * it)</li> <li>object is a L2PlayableInstance : 3000</li> <li>others : 1000</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2Attackable</li><BR>
	 ...
	 * packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>If L2NpcInstance is
	 * autoAttackable, send a Server->Client packet StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP
	 * bar</li> <li>Send a Server->Client packet ValidateLocation to correct the L2NpcInstance position and heading on
	 * the client</li><BR>
	 * <BR>
	 * <B><U> Actions on second click on the L2NpcInstance (Attack it/Intercat with it)</U> :</B><BR>
	 * <BR>
	 * <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>
	 * If L2NpcInstance is autoAttackable, notify the L2PcInstance AI with AI_INTENTION_ATTACK (after a height
	 * verification)</li> <li>If L2NpcInstance is NOT autoAttackable, notify the L2PcInstance AI with
	 * AI_INTENTION_INTERACT (after a distance verification) and show message</li><BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Each group of Server->Client packet must be terminated by a ActionFailed
	 * packet in order to avoid that client wait an other packet</B></FONT><BR>
	 * <BR>
	 ...
	 * packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>If L2NpcInstance is
	 * autoAttackable, send a Server->Client packet StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP
	 * bar</li> <li>Send a Server->Client NpcHtmlMessage() containing the GM console about this L2NpcInstance</li><BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Each group of Server->Client packet must be terminated by a ActionFailed
	 * packet in order to avoid that client wait an other packet</B></FONT><BR>
	 * <BR>
	...
	/**
	 * Send a Server->Client packet NpcHtmlMessage to the L2PcInstance in order to display the message of the
	 * L2NpcInstance.<BR>
	 * <BR>
	 * 
	 ...
	 * <li>if the file exists on the server (page number > 0) : <B>data/html/default/12006-1.htm</B> (npcId-page number)
	 * </li> <li>if the file doesn't exist on the server : <B>data/html/npcdefault.htm</B> (message :
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the quest state in the folder data/scripts/quests/questId/stateId.htm</li> <li>Send a
	 * Server->Client NpcHtmlMessage containing the text of the L2NpcInstance to the L2PcInstance</li> <li>Send a
	 * Server->Client ActionFailed to the L2PcInstance in order to avoid that the client wait another packet</li><BR>
	 * <BR>
	 * 
	 * @param player The L2PcInstance that talk with the L2NpcInstance
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the selected HTML file in function of the npcId and of the page number</li> <li>Send a
	 * Server->Client NpcHtmlMessage containing the text of the L2NpcInstance to the L2PcInstance</li> <li>Send a
	 * Server->Client ActionFailed to the L2PcInstance in order to avoid that the client wait another packet</li><BR>
	 * 
	 * @param player The L2PcInstance that talk with the L2NpcInstance
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the range level in wich player must be to obtain buff</li> <li>If player level is out of range, display a
	 * message and return</li> <li>According to player level cast buff</li><BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> Newbie Helper Buff list is define in sql table helper_buff_list</B></FONT><BR>
	 * <BR>
	 * 
	 * @param player The L2PcInstance that talk with the L2NpcInstance if
	 *			(!FloodProtector.getInstance().tryPerformAction(player.getObjectId(),
	 *			FloodProtector.PROTECTED_USEITEM)) return;
	 */
	 ...
	 * @return boolean
	 */
	/*private boolean showFlagDenyChatWindow(L2PcInstance player, String type)
	{
		String html = HtmCache.getInstance().getHtm("data/html/" + type + "/" + getNpcId() + "-f.htm");
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Create a DecayTask to remove the corpse of the L2NpcInstance after 7 seconds</li> <li>Set target to null and
	 * cancel Attack or Cast</li> <li>Stop movement</li> <li>Stop HP/MP/CP Regeneration task</li> <li>Stop all active
	 * skills effects in progress on the L2Character</li> <li>Send the Server->Client packet StatusUpdate with current
	 * HP and MP to all other L2PcInstance to inform</li> <li>Notify L2Character AI</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2Attackable</li><BR>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the L2NpcInstance from the world when the decay task is launched</li> <li>Decrease its spawn counter</li>
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Remove the L2NpcInstance from the world and update its spawn object</li> <li>Remove all L2Object from
	 * _knownObjects and _knownPlayer of the L2NpcInstance then cancel Attak or Cast and notify AI</li> <li>Remove
	 * L2Object object from _allObjects of L2World</li><BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND Server->Client packets to players</B></FONT><BR>
	 * <BR>
	 */
