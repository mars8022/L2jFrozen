...
import com.l2scoria.gameserver.model.entity.olympiad.OlympiadPeriod;
...
public final class Config
{
	...
	//============================================================
	public static void loadGOFConfig()
	{
		final String GOF = FService.gof_CONFIG_FILE;

		try
		{
			Properties gofSettings = new Properties();
			InputStream is = new FileInputStream(new File(GOF));
			gofSettings.load(is);
			is.close();

			GM_TRADE_RESTRICTED_ITEMS = Boolean.parseBoolean(gofSettings.getProperty("GMTradeRestrictedItems", "False"));
			GM_RESTART_FIGHTING = Boolean.parseBoolean(gofSettings.getProperty("GMRestartFighting", "False"));
			PM_MESSAGE_ON_START = Boolean.parseBoolean(gofSettings.getProperty("PMWelcomeShow", "False"));
			PM_SERVER_NAME  = gofSettings.getProperty("PMServerName", "L2-GoF");
			PM_TEXT1  = gofSettings.getProperty("PMText1", "Have Fun and Nice Stay on");
			PM_TEXT2  = gofSettings.getProperty("PMText2", "Vote for us every 24h");
			CHAR_REPAIR = Boolean.parseBoolean(gofSettings.getProperty("CharacterRepair", "False"));
			PLAYERS_CAN_HEAL_RB = Boolean.parseBoolean(gofSettings.getProperty("PlayersCanHealRb", "True"));
			RBLOCKRAGE = Integer.parseInt(gofSettings.getProperty("RBlockRage", "5000"));
			NEW_PLAYER_EFFECT = Boolean.parseBoolean(gofSettings.getProperty("NewPlayerEffect", "True"));

		}
		catch(Exception e)
		{
			e.printStackTrace();
			throw new Error("Failed to Load " + GOF + " File.");
		}
	}
