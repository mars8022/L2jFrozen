...
public class TradeFilter implements Filter
{
	...
	private String _excludeTradeType;

	// This is example how to exclude consuming of shots and arrows from
	// logging
	// private String _excludeProcess = "Consume";
	// private String _excludeItemType = "Arrow, Shot";
	public boolean isLoggable(LogRecord record)
	{
		if (record.getLoggerName() != "trade") {
			return false;
		}
		...
		if (_excludeTradeType != null)
		{
			// if (record.getParameters() == null ||
			// record.getParameters().length == 0 ||
			// !(record.getParameters()[0]
			// instanceof L2ItemInstance)) return true;
			L2ItemInstance item = (L2ItemInstance) record.getParameters()[0];
			if (!_excludeTradeType.contains(item.getItemType().toString())) {
				return true;
			}
		}
		return _excludeProcess == null && _excludeTradeType == null;
	}
}
