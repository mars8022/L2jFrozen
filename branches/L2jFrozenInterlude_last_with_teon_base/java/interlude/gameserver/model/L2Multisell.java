...
import interlude.gameserver.network.serverpackets.MultiSellList;
...
public class L2Multisell
{
	...
	/**
	 * This will generate the multisell list for the items. There exist various parameters in multisells that affect the way they will appear: 1) inventory only: * if true, only show items of the multisell for which the "primary" ingredients are already in the player's inventory. By "primary" ingredients we mean weapon and armor. * if false, show the entire list. 2) maintain enchantment:
	 ...
	 *
	 * @see interlude.gameserver.network.serverpackets.ServerBasePacket#runImpl()
	 */
	...
	// b) If the entry has no adena ingredients other than the
	// taxIngredient,
	// the resulting
	// amount of adena is appended to the entry
	// c) If the entry already has adena as an entry, the taxIngredient is
	// used
	...
	private MultiSellEntry prepareEntry(MultiSellEntry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantLevel, double taxRate)
	{
		...
		for (MultiSellIngredient ing : templateEntry.getIngredients())
		{
			...
			// if the item is adena, and the item is set true for taxingredient, then the adena count + (adena count * tax) will be the total price paid.
			// if the item is adena, and the item is set false for taxingredient, then the adena count will be the total price paid.
			if (ing.getItemId() == 57)
			{
