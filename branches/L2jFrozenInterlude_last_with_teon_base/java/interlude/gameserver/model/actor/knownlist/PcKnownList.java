...
import interlude.gameserver.network.serverpackets.DeleteObject;
import interlude.gameserver.network.serverpackets.DoorInfo;
import interlude.gameserver.network.serverpackets.DoorStatusUpdate;
import interlude.gameserver.network.serverpackets.DropItem;
...
import interlude.gameserver.network.serverpackets.SpawnItem;
import interlude.gameserver.network.serverpackets.SpawnItemPoly;
import interlude.gameserver.network.serverpackets.StaticObject;
import interlude.gameserver.network.serverpackets.VehicleInfo;
	...
	public boolean addKnownObject(L2Object object, L2Character dropper)
	{
		...
		else
		{
			...
			else if (object instanceof L2Summon)
			{
				...
				// Check if the L2PcInstance is the owner of
				// the Pet
			...
			else if (object instanceof L2PcInstance)
			{
				...
				if (otherPlayer.isInBoat())
				{
					 ...
					 * (yp == 0) { yboat = 1; } if(yboat < 0) { modifier = -modifier; } double anglep = modifier - Math.atan(yp/xp); double finx = Math.cos(anglep - angleboat)Math.sqrt(xpxp +ypyp ) + Math.cos(angleboat)Math.sqrt(xboatxboat +yboatyboat ); double finy = Math.sin(anglep - angleboat)Math.sqrt(xpxp +ypyp ) + Math.sin(angleboat)Math.sqrt(xboatxboat +yboatyboat );
					 * //otherPlayer.getPosition().setWorldPosition(otherPlayer.getBoat().getX() - otherPlayer.getInBoatPosition().x,otherPlayer.getBoat().getY() - otherPlayer.getInBoatPosition().y,otherPlayer.getBoat().getZ()- otherPlayer.getInBoatPosition().z); otherPlayer.getPosition().setWorldPosition((int)finx,(int)finy,otherPlayer.getBoat().getZ()- otherPlayer.getInBoatPosition().z); }
					 */
				}
				else
				{
