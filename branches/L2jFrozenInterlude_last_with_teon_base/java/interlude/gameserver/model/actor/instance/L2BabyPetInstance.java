...
public final class L2BabyPetInstance extends L2PetInstance
{
	...
	public L2BabyPetInstance(int objectId, L2NpcTemplate template, L2PcInstance owner, L2ItemInstance control)
	{
		...
		// look through the skills that this template has and find the weak and
		// strong heal.
		...
		if (skill1 != null)
		{
			if (skill2 == null)
			{
				// duplicate so that the same skill will be used in both normal
	...
	private class Heal implements Runnable
	{
		...
		public void run()
		{
			...
			// if the owner is dead, merely wait for the owner to be
			// resurrected
			// if the pet is still casting from the previous iteration,
			// allow
			// the cast to complete...
			if (!owner.isDead() && !_baby.isCastingNow() && !_baby.isBetrayed())
			{
				// casting automatically stops any other action (such as
				// autofollow or a move-to).
				// We need to gather the necessary info to restore the previous
				// state.
				...
				// etc).
				// if the status has actually been changed, revert it. Else,
				// allow the pet to
				// continue whatever it was trying to do.
				// NOTE: This is important since the pet may have been told to
				// attack a target.
				// reverting the follow status will abort this attack! While
				// aborting the attack
				// in order to heal is natural, it is not acceptable to abort
