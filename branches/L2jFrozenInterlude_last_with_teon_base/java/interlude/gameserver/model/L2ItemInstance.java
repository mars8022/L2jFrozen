...
public final class L2ItemInstance extends L2Object
{
	...
	/** Is item rented? */
	private boolean _donatorRentedItem;
	...
	/*
	 * (non-Javadoc)
	 * @see interlude.gameserver.model.L2Object#onAction(interlude.gameserver.model.L2PcInstance) also check constraints: only soloing castle owners may pick up mercenary tickets of their castle
	 */
	@Override
	public void onAction(L2PcInstance player)
	{
		...
		if (castleId != -1)
		{
			...
			else if ((player.getClanPrivileges() & L2Clan.CP_CS_MERCENARIES) != L2Clan.CP_CS_MERCENARIES)
			{
				player.sendMessage("You don't have the right to pickup mercenaries.");
				canPickup = false;
			}
		}
		if (canPickup)
		...
		if (player != null)
		{
			...
			if (_mana == 0) // The life time has expired
			{
				...
				if (getLocation() != ItemLocation.WAREHOUSE)
				{
					...
					// remove cupid's bow for event skill
					if (isCupidBow())
					{
	...
	public static L2ItemInstance restoreFromDb(int objectId)
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT owner_id, object_id, item_id, count, enchant_level, loc, loc_data, price_sell, price_buy, custom_type1, custom_type2, mana_left, donatorRentTime FROM items WHERE object_id = ?");
			...
			if (rs.next())
			{
				...
				long donatorRentTime = rs.getLong("donatorRentTime");
				...
				// is item - rented donator's item?
				if (donatorRentTime > 0)
				{
					item.setCrystallizable(false); // donator can't crystallize rented item
					item.setSellable(false); // donator can's sell rented item
					item.setDropable(false); // donator can't drop rented item
					item.setDestroyable(false); // donator can't destroy rented item
					item.setTradeable(false); // donator can't trade rented item
				}
	...
	{
		return _donatorRentedItem;
	}

	public final void setDonatorRented(boolean value)
	{
