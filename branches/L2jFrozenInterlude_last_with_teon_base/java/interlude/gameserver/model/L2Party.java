...
import interlude.gameserver.network.serverpackets.PartySmallWindowAdd;
import interlude.gameserver.network.serverpackets.PartySmallWindowAll;
import interlude.gameserver.network.serverpackets.PartySmallWindowDelete;
import interlude.gameserver.network.serverpackets.PartySmallWindowDeleteAll;
import interlude.gameserver.network.serverpackets.PartySmallWindowUpdate;
...
public class L2Party
{
	...
	// Logger.getLogger(L2Party.class.getName());
	 ...
	 */
	/*
	 * private L2PcInstance getNextLooter() { _itemLastLoot++; if (_itemLastLoot > getPartyMembers().size() -1) _itemLastLoot = 0; return (getPartyMembers().size() > 0) ? getPartyMembers().get(_itemLastLoot) : null; }
	 */
	 ...
	 */
	public void updatePartyMembersWindow()
	{
		for (L2PcInstance member : getPartyMembers())
		{
		...
		if (player != null && !player.isInDuel())
		{
			if (getPartyMembers().contains(player))
			{
				...
				else
				{
					...
					// update party windows (new leader)
					updatePartyMembersWindow();
				}
			}
			else
			{
	 ...
	 */
	/*
	 * [DEPRECATED] private void dissolveParty() { SystemMessage msg = new SystemMessage(SystemMessageId.PARTY_DISPERSED); for(int i = 0; i < _members.size(); i++) { L2PcInstance temp = _members.get(i); temp.sendPacket(msg); temp.sendPacket(new PartySmallWindowDeleteAll()); temp.setParty(null); } }
	 */
	/**
	...
	public void distributeXpAndSp(long xpReward, int spReward, List<L2PlayableInstance> rewardedMembers, int topLvl)
	{
		...
		synchronized (rewardedMembers)
		{
			for (L2Character member : rewardedMembers)
			{
				...
				// Pets that leech xp from the owner (like babypets) do not get
				// rewarded directly
