...
import interlude.gameserver.model.L2Macro.L2MacroCmd;
import interlude.gameserver.model.actor.instance.L2PcInstance;
import interlude.gameserver.network.serverpackets.SendMacroList;
...
public class MacroList
{
	...
	private void registerMacroInDb(L2Macro macro)
	{
		...
		try
		{
			...
			if (macro.name.length() > 40)
			{
				_owner.sendMessage("Sorry, but macros name is too long.");
				allowRegister = false;
			}
			if (macro.descr.length() > 80)
			{
				_owner.sendMessage("Sorry, but macros description is too long.");
				allowRegister = false;
			}
			if (sb.toString().length() > 255)
			{
				_owner.sendMessage("Sorry, but macros command list is too long.");
				allowRegister = false;
			}
			if (allowRegister)
			{
