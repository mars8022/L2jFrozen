...
import interlude.gameserver.model.L2PetDataTable;
...
import interlude.gameserver.model.item.Item;
import interlude.gameserver.skills.SkillsEngine;
import interlude.gameserver.templates.L2Armor;
import interlude.gameserver.templates.L2ArmorType;
import interlude.gameserver.templates.L2EtcItem;
...
public class ItemTable
{
	...
	static
	{
		...
		_weaponTypes.put("bigblunt", L2WeaponType.BIGBLUNT); // Two handed
		// blunt
	...
	private static final String[] SQL_ITEM_SELECTS = {
			...
			"SELECT item_id, name, bodypart, crystallizable, weight, soulshots, spiritshots," + " material, crystal_type, p_dam, rnd_dam, weaponType, critical, hit_modify, avoid_modify," + " shield_def, shield_def_rate, atk_speed, mp_consume, m_dam, duration, price, crystal_count,"
	...
	/*
	 * private void fillEtcItemsTable() { for (Item itemInfo : itemData.values()) { L2EtcItem item = SkillsEngine.getInstance().loadEtcItem(itemInfo.id, itemInfo.type, itemInfo.name, itemInfo.set); if (item == null) { item = new L2EtcItem((L2EtcItemType)itemInfo.type, itemInfo.set); } _etcItems.put(item.getItemId(), item); } } private void fillArmorsTable() { List<L2Armor> armorList =
	 * SkillsEngine.getInstance().loadArmors(armorData); /for (Item itemInfo : armorData.values()) { L2Armor armor = SkillsEngine.getInstance().loadArmor(itemInfo.id, itemInfo.type, itemInfo.name, itemInfo.set); if (armor == null) armor = new L2Armor((L2ArmorType)itemInfo.type, itemInfo.set); _armors.put(armor.getItemId(), armor); } } private void FillWeaponsTable() { for (Item itemInfo :
	 * weaponData.values()) { L2Weapon weapon = SkillsEngine.getInstance().loadWeapon(itemInfo.id, itemInfo.type, itemInfo.name, itemInfo.set); if (weapon == null) weapon = new L2Weapon((L2WeaponType)itemInfo.type, itemInfo.set); _weapons.put(weapon.getItemId(), weapon); } }
	 */
	/**
