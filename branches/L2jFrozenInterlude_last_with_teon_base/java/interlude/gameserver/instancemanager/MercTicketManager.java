...
import interlude.gameserver.model.AutoChatHandler;
...
/**
 * @author yellowperil & Fulminus This class is similar to the SiegeGuardManager, except it handles the loading of the mercenary tickets that are dropped on castle floors by the castle lords. These tickets (aka badges) need to be readded after each server reboot except when the server crashed in the middle of an ongoig siege. In addition, this class keeps track of the added tickets, in order to
 *         properly limit the number of mercenaries in each castle and the number of mercenaries from each mercenary type. Finally, we provide auxilary functions to identify the castle in which each item (and its corresponding NPC) belong to, in order to help avoid mixing them up.
 */
public class MercTicketManager
{
	...
	private final void load()
	{
		...
		try
		{
			...
			int itemId;
			int x, y, z;
			// start index to begin the search for the itemId corresponding
			// to
			// this NPC
			// this will help with:
			// a) skip unnecessary iterations in the search loop
			// b) avoid finding the wrong itemId whenever tickets of
			// different
			// spawn the same npc!
			int startindex = 0;
			while (rs.next())
	...
	/**
	 * addTicket actions 1) find the npc that needs to be saved in the mercenary spawns, given this item 2) Use the passed character's location info to add the spawn 3) create a copy of the item to drop in the world returns the id of the mercenary npc that was added to the spawn returns -1 if this fails.
	 */
	public int addTicket(int itemId, L2PcInstance activeChar, String[] messages)
	{
		...
		for (int i = 0; i < ITEM_IDS.length; i++)
		{
			if (ITEM_IDS[i] == itemId) // Find the index of the item used
			{
				...
				dropticket.setLocation(L2ItemInstance.ItemLocation.INVENTORY);
				dropticket.dropMe(null, x, y, z);
				dropticket.setDropTime(0); // avoids it from beeing removed
				// by
