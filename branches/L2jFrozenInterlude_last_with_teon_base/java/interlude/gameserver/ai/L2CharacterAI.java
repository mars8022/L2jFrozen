...
import static interlude.gameserver.ai.CtrlIntention.AI_INTENTION_MOVE_TO;
import static interlude.gameserver.ai.CtrlIntention.AI_INTENTION_PICK_UP;
import static interlude.gameserver.ai.CtrlIntention.AI_INTENTION_REST;
...
import interlude.gameserver.network.serverpackets.AutoAttackStop;
...
public class L2CharacterAI extends AbstractAI
{
	...
	protected void onIntentionActive()
	{
		...
		if (getIntention() != AI_INTENTION_ACTIVE)
		{
			...
			setCastTarget(null);
			setAttackTarget(null);
			// Stop the actor movement server side AND client side by
			// sending
			// Server->Client packet StopMove/StopRotation (broadcast)
			clientStopMoving(null);
			// Stop the actor auto-attack client side by sending
			// Server->Client
			// packet AutoAttackStop (broadcast)
			clientStopAutoAttack();
			// Also enable random animations for this L2Character if allowed
			// This is only for mobs - town npcs are handled in their
			// constructor
	...
	/*
	 * (non-Javadoc)
	 * @see interlude.gameserver.ai.AbstractAI#onIntentionMoveToInABoat(interlude.gameserver.model.L2CharPosition, interlude.gameserver.model.L2CharPosition)
	 */
	@Override
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>Break an attack and send Server->Client ActionFailed packet and a System Message to the L2Character</li> <li>Break a cast and send Server->Client
	 * ActionFailed packet and a System Message to the L2Character</li> <li>Launch actions corresponding to the Event onAttacked (only for L2AttackableAI after the stunning periode)</li> <BR>
	 * <BR>
	 */
	@Override
	 ...
	 * <BR>
	 * <li>Stop the actor auto-attack client side by sending Server->Client packet AutoAttackStop (broadcast)</li> <li>Stop the actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation (broadcast)</li> <li>Break an attack and send Server->Client ActionFailed packet and a System Message to the L2Character</li> <li>Break a cast and send Server->Client
	 * ActionFailed packet and a System Message to the L2Character</li> <BR>
	 * <BR>
	 */
	@Override
	...
	protected void onEvtArrived()
	{
		...
		if (_accessor.getActor().moveToNextRoutePoint()) return;
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>If the object was targeted and the Intention was AI_INTENTION_INTERACT or AI_INTENTION_PICK_UP, set the Intention to AI_INTENTION_ACTIVE</li> <li>If the object was targeted to attack, stop the auto-attack, cancel target and set the Intention to AI_INTENTION_ACTIVE</li> <li>If the object was targeted to cast, cancel target and set the Intention to AI_INTENTION_ACTIVE</li> <li>If the
	 * object was targeted to follow, stop the movement, cancel AI Follow Task and set the Intention to AI_INTENTION_ACTIVE</li> <li>If the targeted object was the actor , cancel AI target, stop AI Follow Task, stop the movement and set the Intention to AI_INTENTION_IDLE</li> <BR>
	 * <BR>
	 */
	@Override
	protected void onEvtForgetObject(L2Object object)
	{
		...
		// AI_INTENTION_INTERACT or AI_INTENTION_PICK_UP, set the Intention to AI_INTENTION_ACTIVE
		if (getTarget() == object)
		{
			setTarget(null);
			if (getIntention() == AI_INTENTION_INTERACT)
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the distance between the current position of the L2Character and the target (x,y)</li> <li>If the distance > offset+20, move the actor (by running) to Pawn server side AND client side by sending Server->Client packet MoveToPawn (broadcast)</li> <li>If the distance <= offset+20, Stop the actor movement server side AND client side by sending Server->Client packet StopMove/StopRotation
	 * (broadcast)</li> <BR>
	 * <BR>
    ...
    protected class TargetAnalysis
    {
        ...
        public void update(L2Character target)
        {
            ...
            if (target == character && Rnd.nextInt(100) > 25) return;
