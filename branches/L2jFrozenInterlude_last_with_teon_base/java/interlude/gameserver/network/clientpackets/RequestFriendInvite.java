...
import interlude.gameserver.network.serverpackets.AskJoinFriend;
...
public final class RequestFriendInvite extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if (friend.isNoob() && activeChar.isKoof() && Config.ENABLE_FACTION_KOOFS_NOOBS)
		{
			// Target is wrong.
		...
		else if (friend.isKoof() && activeChar.isNoob() && Config.ENABLE_FACTION_KOOFS_NOOBS)
		{
			// Target is wrong.
			sm = new SystemMessage(SystemMessageId.TARGET_IS_INCORRECT);
			activeChar.sendPacket(sm);
