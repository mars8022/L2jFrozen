...
public class ExShowSeedInfo extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		for (SeedProduction seed : _seeds)
		{
			...
			writeD(L2Manor.getInstance().getSeedLevel(seed.getId())); // Seed
			// Level
			writeC(1); // reward 1 Type
			writeD(L2Manor.getInstance().getRewardItemBySeed(seed.getId(), 1)); // Reward
			// 1
			// Type
			// Item
			// Id
			writeC(1); // reward 2 Type
			writeD(L2Manor.getInstance().getRewardItemBySeed(seed.getId(), 2)); // Reward
			// 2
			// Type
			// Item
			// Id
