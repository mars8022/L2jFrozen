...
public class PackageSendableList extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		for (L2ItemInstance item : _items) // format inside the for taken
		// from
		// SellList part use should be about
		// the same
		{
			...
			writeH(0x00);
			writeH(0x00);
			writeD(item.getObjectId()); // some item identifier later used by
			// client to answer (see
			// RequestPackageSend) not item id nor
			// object id maybe some freight system
			// id??
		}
	}
