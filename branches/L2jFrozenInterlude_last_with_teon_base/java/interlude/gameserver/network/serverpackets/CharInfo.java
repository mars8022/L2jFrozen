	...
	protected final void writeImpl()
	{
		...
		if (_activeChar.getPoly().isMorphed())
		{
			...
			if (template != null)
			{
				...
				writeD(_activeChar.getPoly().getPolyId() + 1000000); // npctype
				// id
		...
		else
		{
			...
			// In UserInfo leader rights and siege flags, but here found
			// nothing??
			// Therefore RelationChanged packet with that info is required
			writeD(0);
			writeC(_activeChar.isSitting() ? 0 : 1); // standing = 1 sitting
			// = 0
			writeC(_activeChar.isRunning() ? 1 : 0); // running = 1 walking =
			// 0
			writeC(_activeChar.isInCombat() ? 1 : 0);
			writeC(_activeChar.isAlikeDead() ? 1 : 0);
			if (gmSeeInvis)
