...
public class SevenSignsFestival implements SpawnListener
{
	...
	/**
	 * These length settings are important! :) All times are relative to the ELAPSED time (in ms) since a festival
	 * begins. Festival manager start is the time after the server starts to begin the first festival cycle. The cycle
	 * length should ideally be at least 2x longer than the festival length. This allows ample time for players to
	 * sign-up to participate in the festival. The intermission is the time between the festival participants being
	 * moved to the "arenas" and the spawning of the first set of mobs. The monster swarm time is the time before the
	 * monsters swarm to the center of the arena, after they are spawned. The chest spawn time is for when the bonus
	 * festival chests spawn, usually towards the end of the festival.
	 */
	...
	*/
	...
	private class L2DarknessFestival
	{
		...
		{
		    if (Config.DEBUG)
		        LOGGER.info("SevenSignsFestival: Modifying spawn rate of festival mobs to " + respawnDelay + " ms for festival " + SevenSigns.getCabalShortName(_cabal) + " (" + getFestivalName(_levelRange) + ")");
