...
import interlude.gameserver.pathfinding.utils.BinaryNodeHeap;
import interlude.gameserver.pathfinding.utils.FastNodeList;
...
public abstract class PathFinding
{
	...
	public List<AbstractNodeLoc> search(Node start, Node end)
	{
		...
		// Drawback is not having higher cost for diagonal movement (means funny
		// routes)
		// Could be optimized e.g. not to calculate backwards as far as
		// forwards.
	...
	public List<AbstractNodeLoc> searchByClosest(Node start, Node end)
	{
		...
		// to go backwards/sideways but when moving generally forwards, this is
		// extra fast
		// and accurate. And can reach insane distances (try it with 800
		// nodes..).
		// Minimum required node count would be around 300-400.
		// Generally returns a bit (only a bit) more intelligent looking routes
		// than
		// the basic version. Not a true distance image (which would increase
		// CPU
	...
	public List<AbstractNodeLoc> searchAStar(Node start, Node end)
	{
		...
		// List of Visited Nodes
		FastNodeList visited = new FastNodeList(800);// TODO! Add limit to
		// cfg
		// List of Nodes to Visit
