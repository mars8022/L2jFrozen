 ...
 */
package interlude.gameserver.model.item;
...
import interlude.gameserver.model.TradeList.TradeItem;
...
public class PcInventory extends Inventory
{
	...
	{
		return getUniqueItems(allowAdena, allowAncientAdena, Adventurer, true);
	}

	public L2ItemInstance[] getUniqueItems(boolean allowAdena, boolean allowAncientAdena, boolean Adventurer, boolean onlyAvailable)
	{
		...
		for (L2ItemInstance item : _items)
		{
			...
			if (!isDuplicate && (!onlyAvailable || item.getItem().isSellable() && item.isAvailable(getOwner(), false, Adventurer))) {
				list.add(item);
			}
		}
	...
	public L2ItemInstance[] getUniqueItemsByEnchantLevel(boolean allowAdena, boolean allowAncientAdena, boolean onlyAvailable)
	{
		...
		for (L2ItemInstance item : _items)
		{
			...
			if (!isDuplicate && (!onlyAvailable || item.getItem().isSellable() && item.isAvailable(getOwner(), false, false))) {
				list.add(item);
			}
		}
		return list.toArray(new L2ItemInstance[list.size()]);
