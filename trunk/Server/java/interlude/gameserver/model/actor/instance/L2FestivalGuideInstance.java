		...
		else if (command.startsWith("Festival"))
		{
			...
			switch (val)
			{
				case 1: // Become a Participant
					// Check if the festival period is active, if not then
					...
					// Check if all the party members are in the required
					// level range.
					...
					// TODO: Check if the player has delevelled by comparing
					// their skill levels.
					/*
				...
				case 3: // Score Registration
					// Check if the festival period is active, if not then
					// don't register the score.
					if (SevenSigns.getInstance().isSealValidationPeriod())
					{
						showChatWindow(player, 3, "a", false);
						return;
					}
					// Check if a festival is in progress, if it is don't
					// register the score.
					...
					// Check if this player was the party leader in the
					// festival.
					...
					int offeringCount = 0;
					// Check if the player collected any blood offerings
					// during the festival.
					...
					// Send message that the contribution score has
					// increased.
