...
import interlude.gameserver.datatables.HelperBuffTable;
...
import interlude.gameserver.instancemanager.games.Lottery;
...
import interlude.gameserver.model.MobGroupTable;
import interlude.gameserver.model.NpcInventory;
import interlude.gameserver.model.L2Skill.SkillType;
import interlude.gameserver.model.actor.knownlist.NpcKnownList;
import interlude.gameserver.model.actor.stat.NpcStat;
import interlude.gameserver.model.actor.status.NpcStatus;
...
import interlude.gameserver.model.zone.type.L2TownZone;
import interlude.gameserver.network.L2GameClient;
import interlude.gameserver.network.SystemMessageId;
import interlude.gameserver.network.serverpackets.ActionFailed;
import interlude.gameserver.network.serverpackets.ExShowVariationCancelWindow;
import interlude.gameserver.network.serverpackets.ExShowVariationMakeWindow;
...
import interlude.gameserver.templates.L2HelperBuff;
...
public class L2NpcInstance extends L2Character
{
	...
	public boolean isEventMob = false, _isEventMobTvT = false, _isEventMobDM = false, _isEventMobCTF = false, _isCTF_throneSpawn = false, _isCTF_Flag = false, _isEventMobFOS = false, _isFOS_Artifact = false;
		...
		public void run()
		{
			try
			{
				...
				if (isMob())
					...
					if (getAI().getIntention() != AI_INTENTION_ACTIVE) return;
	 ...
	 * <BR>
	 * <li>Set the L2NpcInstance as target of the L2PcInstance player (if necessary)</li> <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>If L2NpcInstance is autoAttackable, send a Server->Client packet StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP bar</li> <li>Send a Server->Client packet ValidateLocation to
	 * correct the L2NpcInstance position and heading on the client</li> <BR>
	 * <BR>
	...
	public void onAction(L2PcInstance player)
	{
		...
		if (this != player.getTarget())
		{
			...
			{
				// Send a Server->Client packet MyTargetSelected to the L2PcInstance playe The player.getLevel() - getLevel() permit to display the correct color in the select window
	 ...
	 * <BR>
	 * <li>Set the L2NpcInstance as target of the L2PcInstance player (if necessary)</li> <li>Send a Server->Client packet MyTargetSelected to the L2PcInstance player (display the select window)</li> <li>If L2NpcInstance is autoAttackable, send a Server->Client packet StatusUpdate to the L2PcInstance in order to update L2NpcInstance HP bar</li> <li>Send a Server->Client NpcHtmlMessage() containing
	 * the GM console about this L2NpcInstance</li> <BR>
	 * <BR>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : Each group of Server->Client packet must be terminated by a ActionFailed packet in order to avoid that client wait an other packet</B></FONT><BR>
	 * <BR>
	 * <B><U> Example of use </U> :</B><BR>
	...
	public void onActionShift(L2GameClient client)
	{
		...
		if (player.getAccessLevel() >= Config.GM_ACCESSLEVEL)
		{
			...
			html1.append("<tr><td><button value=\"Show Skillist\" action=\"bypass -h admin_show_skilllist_npc " + getTemplate().getNpcId()
					+ "\" width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><td></td></tr>");
		...
		// if (canInteract(player))
		{
			...
			else if (command.startsWith("open_gate"))
			{
				...
				while (st.hasMoreTokens())
				{
					...
					{
						_log.fine("Door Id does not exist.(" + doorId + ")");
					}
				}
				return;
			}
			...
			else if (command.startsWith("event"))
			{
				...
				if (getNpcId()==35596)
					RainbowSpringSiegeManager.getInstance().exchangeItem(player,val);
			}
			else if (command.startsWith("HotSpringsArena"))
			{
				if (!RainbowSpringSiegeManager.getInstance().enterOnArena(player))
				{
					NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
	...
	/**
	 * Throws an action command to L2BufferOpenInstance.<br>
	 *
	 * @param player
	 *            --> Target player
	 * @param buffTemplate
	...
	public void makeBuffs(L2PcInstance player, String buffTemplate)
	{
		...
		if (_templateId > 0)
			L2BufferOpenInstance.makeBuffs(player, _templateId, this, true);
	}
	...
	public void showQuestWindow(L2PcInstance player, String questId)
	{
		...
		if (qs != null)
		{
			...
			if (!qs.getQuest().notifyTalk(this, qs)) return;
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Create a DecayTask to remove the corpse of the L2NpcInstance after 7 seconds</li> <li>Set target to null and cancel Attack or Cast</li> <li>Stop movement</li> <li>Stop HP/MP/CP Regeneration task</li> <li>Stop all active skills effects in progress on the L2Character</li> <li>Send the Server->Client packet StatusUpdate with current HP and MP to all other L2PcInstance to inform</li> <li>
	 * Notify L2Character AI</li> <BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
	 * <li>L2Attackable</li> <BR>
	...
	public void deleteMe()
	{
		if (getWorldRegion() != null)
		{
			...
			// FIXME this is just a temp hack, we should find a better
			// solution
		}
		try
		{
