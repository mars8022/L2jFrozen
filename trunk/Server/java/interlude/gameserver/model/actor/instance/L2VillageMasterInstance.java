...
import interlude.gameserver.model.base.ClassType;
import interlude.gameserver.model.base.PlayerClass;
import interlude.gameserver.model.base.PlayerRace;
...
public final class L2VillageMasterInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("Subclass"))
		{
            ...
            */ 
            ...
            if (!Restrictions(player) || player.isAllSkillsDisabled())
				return;
			...
			switch (cmdChoice)
			{
				...
				case 5: // Change Class - Action
                    ...
                    if (!ItemRestriction(player))
                    	return; //Check the player for items during subclass..to avoid bugs
                    if (Config.NOT_WEAPON_CHANCE_SUBCLASS)
                    {
	...
	private final Set<PlayerClass> getAvailableSubClasses(L2PcInstance player)
	{
		...
		/**
		 * If the race of your main class is Elf or Dark Elf, you may not select each class as a subclass to the other class, and you may not select Overlord and Warsmith class as a subclass. You may not select a similar class as the subclass. The occupations classified as similar classes are as follows: Treasure Hunter, Plainswalker and Abyss Walker Hawkeye, Silver Ranger and Phantom Ranger
		 * Paladin, Dark Avenger, Temple Knight and Shillien Knight Warlocks, Elemental Summoner and Phantom Summoner Elder and Shillien Elder Swordsinger and Bladedancer Sorcerer, Spellsinger and Spellhowler
		 */
    ...
    private boolean Restrictions(L2PcInstance player)
    {
		if (!isInsideRadius(player, 150, false, false))
		{
			player.sendPacket(ActionFailed.STATIC_PACKET);
        ...
        if (player.isInOlympiadMode() || player.isInCombat() || player.isSilentMoving() || player.isEnchanting() 
        		|| player.isCastingNow()|| player.isDead() || player.isAlikeDead()/* || player.isAllSkillsDisabled()*/)
		{
			player.sendPacket(ActionFailed.STATIC_PACKET);
			player.sendPacket(new SystemMessage(SystemMessageId.YOU_ARE_NOT_AUTHORIZED_TO_DO_THAT));
			return false;
