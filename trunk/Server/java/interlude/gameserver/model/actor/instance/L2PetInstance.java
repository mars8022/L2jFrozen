...
import interlude.gameserver.model.L2PetData;
...
import interlude.gameserver.model.actor.stat.PetStat;
...
import interlude.gameserver.network.serverpackets.PetInventoryUpdate;
import interlude.gameserver.network.serverpackets.PetItemList;
import interlude.gameserver.network.serverpackets.PetStatusShow;
...
public class L2PetInstance extends L2Summon
{
	...
	class FeedTask implements Runnable
	{
		public void run()
		{
			try
			{
				...
				{
					// go back to pet control item, or simply said, unsummon
					// it
					setCurrentFed(0);
					stopFeed();
					unSummon(getOwner());
	...
	public void deleteMe(L2PcInstance owner)
	{
		...
		destroyControlItem(owner); // this should also delete the pet from
		// the
		// db
	}
	...
	/**
	 * Save Pet's ObjectId and Owner ObjectId to database
	 */
	private static void saveObjectInfo(int ownerId, int petId)
	{
		deleteObjectInfo(ownerId);
		java.sql.Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("INSERT INTO pets_objects VALUES(?,?)");
		...
		{
			_logPet.warning("Could not save petId, ownerId data: " + e);
		}
		finally
		{
			try
	...
	/**
	 * Delete Pet's ObjectId and Owner ObjectId to database
	 */
	private static void deleteObjectInfo(int ownerId)
	{
		java.sql.Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM pets_objects WHERE ownerId=?");
		...
		{
			_logPet.warning("Could not delete petId, ownerId data: " + e);
		}
		finally
		{
			try
	...
	/**
	 * Restore Pet's data from database
	 */
	private static L2PetInstance restore(L2ItemInstance control, L2NpcTemplate template, L2PcInstance owner)
	{
		...
		try
		{
			...
			rset.close();
			statement.close();
			// === PET ITEMS ON CRASH RESTORE SYSTEM ================
			saveObjectInfo(owner.getObjectId(), pet.getObjectId());
			// ======================================================
			return pet;
		}
		catch (Exception e)
	...
	public void store()
	{
		...
		if (!isRespawned())
		{
			...
			// === PET ITEMS ON CRASH RESTORE SYSTEM ================
			saveObjectInfo(getOwner().getObjectId(), getObjectId());
			// ======================================================
		}
		else
		{
			...
			// === PET ITEMS ON CRASH RESTORE SYSTEM ===================
			deleteObjectInfo(getOwner().getObjectId());
			// =========================================================
		}
		java.sql.Connection con = null;
		try
	...
	public synchronized void startFeed(boolean battleFeed)
	{
		...
		if (!isDead())
		{
			...
			// pet feed time must be different than 0. Changing time to
			// bypass
			// divide by 0
			if (_feedTime <= 0)
			{
	...
	public final void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		// Prevents the double spam of system messages, if the target is the
		// owning player.
