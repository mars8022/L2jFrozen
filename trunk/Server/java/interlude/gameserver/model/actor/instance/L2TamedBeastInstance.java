...
// The (mostly optional) AI on feeding the spawn is handled by the datapack ai
// script
public final class L2TamedBeastInstance extends L2FeedableBeastInstance
{
	...
	private static final int DURATION_INCREASE_INTERVAL = 20000; // 20
	// secs
	// (gained
	// upon
	// feeding)
	...
	public void setOwner(L2PcInstance owner)
	{
		if (owner != null)
		{
			...
			// instead of calculating this value each time, let's get this
			// now and pass it on
	...
	public void onOwnerGotAttacked(L2Character attacker)
	{
		...
		// if the owner is too far away, stop anything else and immediately run
		// towards the owner.
		...
		// for HP levels between 80% and 50%, do not react to attack events (so
		// that MP can regenerate a bit)
		// for lower HP ranges, heal or recharge the owner with 1 skill use per
		// attack.
		else if (HPRatio < 0.5)
		{
			int chance = 1;
	 ...
	 *
	 * @see interlude.gameserver.model.L2Character#doCast(interlude.gameserver.model.L2Skill)
	 */
	...
	private class CheckDuration implements Runnable
	{
		...
		public void run()
		{
			...
			// I tried to avoid this as much as possible...but it seems I
			// can't avoid hardcoding
			// ids further, except by carrying an additional variable just
			// for these two lines...
			...
			if (item != null && item.getCount() >= 1)
			{
				...
				// emulate a call to the owner using food, but bypass all checks
				// for range, etc
				// this also causes a call to the AI tasks handling feeding,
				// which may call onReceiveFood as required.
	...
	private class CheckOwnerBuffs implements Runnable
	{
		...
		public void run()
		{
			...
			// if the owner is too far away, stop anything else and
			// immediately run towards the owner.
			...
			// if the owner has less than 60% of this beast's available
			// buff, cast a random buff
