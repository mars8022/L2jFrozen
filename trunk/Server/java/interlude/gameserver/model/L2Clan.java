...
import interlude.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
...
import interlude.gameserver.network.serverpackets.PledgeShowMemberListDeleteAll;
import interlude.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import interlude.gameserver.network.serverpackets.PledgeSkillListAdd;
...
public class L2Clan
{
	...
	public void setNewLeader(L2ClanMember member, L2PcInstance activeChar)
	{
		...
		// transfer clan privill doesn't remove crown from exLeader
		for (L2ItemInstance item : exLeader.getInventory().getItems())
		{
			if (item.getItemId() == 6841)
			{
				exLeader.destroyItem("Removing Crown", item, exLeader, true);
				exLeader.getInventory().updateDatabase();
			}
		}
		...
		if (exMember.isOnline())
		{
			...
			if (player.isClanLeader())
			{
				...
				player.setClanCreateExpiryTime(System.currentTimeMillis() 
						+ Config.ALT_CLAN_CREATE_DAYS * 86400000L); // 24*60*60*1000 = 86400000
			}
			// remove clan skills from players in Olympiad
	...
	public void addSkillEffects(L2PcInstance cm)
	{
		...
		for (L2Skill skill : _skills.values())
		{
			// TODO add skills according to members class( in ex. don't add
			// Clan Agillity skill's effect to lower class then Baron)
	...
	{
		private int _rankId;
		private int _party;// TODO find out what this stuff means and
		// implement
		// it
	...
	public boolean checkClanJoinCondition(L2PcInstance activeChar, L2PcInstance target, int pledgeType)
	{
		...
		if (activeChar.isKoof() && target.isNoob() && Config.ENABLE_FACTION_KOOFS_NOOBS)
		{
			activeChar.sendPacket(new SystemMessage(SystemMessageId.YOU_HAVE_INVITED_THE_WRONG_TARGET));
			return false;
		}
		if (activeChar.isNoob() && target.isKoof() && Config.ENABLE_FACTION_KOOFS_NOOBS)
		{
			activeChar.sendPacket(new SystemMessage(SystemMessageId.YOU_HAVE_INVITED_THE_WRONG_TARGET));
			return false;
		}
	...
	public void createAlly(L2PcInstance player, String allyName)
	{
		...
		if (Config.DEBUG) _log.fine(player.getObjectId() 
				+ "(" + player.getName() 
		...
		switch (getLevel())
		{
			case 0:
			{
				...
				if (player.getSp() >= Config.CLAN_SP_FIRST && player.getAdena() >= Config.CLAN_RAISE_FIRST_COST)
				{
					...
					{
						player.setSp(player.getSp() - Config.CLAN_SP_FIRST);
						SystemMessage sp = new SystemMessage(SystemMessageId.SP_DECREASED_S1);
						sp.addNumber(Config.CLAN_SP_FIRST);
						player.sendPacket(sp);
						sp = null;
			...
			case 1:
			{
				...
				if (player.getSp() >= Config.CLAN_SP_SEC && player.getAdena() >= Config.CLAN_RAISE_SEC_COST)
				{
					...
					{
						player.setSp(player.getSp() - Config.CLAN_SP_SEC);
						SystemMessage sp = new SystemMessage(SystemMessageId.SP_DECREASED_S1);
						sp.addNumber(Config.CLAN_SP_SEC);
						player.sendPacket(sp);
						sp = null;
						increaseClanLevel = true;
					}
			...
			case 2:
			{
				...
				if (player.getSp() >= Config.CLAN_SP_THIRD && player.getInventory().getItemByItemId(1419) != null)
				{
					...
					{
						player.setSp(player.getSp() - Config.CLAN_SP_THIRD);
						SystemMessage sp = new SystemMessage(SystemMessageId.SP_DECREASED_S1);
						sp.addNumber(Config.CLAN_SP_THIRD);
						player.sendPacket(sp);
						sp = null;
			...
			case 3:
			{
				...
				if (player.getSp() >= Config.CLAN_SP_FORTH && player.getInventory().getItemByItemId(3874) != null)
				{
					...
					{
						player.setSp(player.getSp() - Config.CLAN_SP_FORTH);
						SystemMessage sp = new SystemMessage(SystemMessageId.SP_DECREASED_S1);
						sp.addNumber(Config.CLAN_SP_FORTH);
						player.sendPacket(sp);
						sp = null;
						SystemMessage sm = new SystemMessage(SystemMessageId.DISSAPEARED_ITEM);
						sm.addItemName(3874);
			...
			case 4:
			{
				...
				if (player.getSp() >= Config.CLAN_SP_FIFTH && player.getInventory().getItemByItemId(3870) != null)
				{
					...
					{
						player.setSp(player.getSp() - Config.CLAN_SP_FIFTH);
						SystemMessage sp = new SystemMessage(SystemMessageId.SP_DECREASED_S1);
						sp.addNumber(Config.CLAN_SP_FIFTH);
						player.sendPacket(sp);
						sp = null;
						SystemMessage sm = new SystemMessage(SystemMessageId.DISSAPEARED_ITEM);
						sm.addItemName(3870);
			...
			case 5:
				if (getReputationScore() >= Config.CLAN_REPUTATION_FIRST && getMembersCount() >= Config.CLAN_MEMBERS_FIRST)
				{
					...
					cr.addNumber(Config.CLAN_REPUTATION_FIRST);
			...
			case 6:
				if (getReputationScore() >= Config.CLAN_REPUTATION_SEC && getMembersCount() >= Config.CLAN_MEMBERS_SEC)
				{
					...
					SystemMessage cr = new SystemMessage(SystemMessageId.S1_DEDUCTED_FROM_CLAN_REP);
					cr.addNumber(Config.CLAN_REPUTATION_SEC);
					player.sendPacket(cr);
					cr = null;
					increaseClanLevel = true;
				}
			...
			case 7:
				if (getReputationScore() >= Config.CLAN_REPUTATION_THIRD && getMembersCount() >= Config.CLAN_MEMBERS_THIRD)
				{
					...
					SystemMessage cr = new SystemMessage(SystemMessageId.S1_DEDUCTED_FROM_CLAN_REP);
					cr.addNumber(Config.CLAN_REPUTATION_THIRD);
					player.sendPacket(cr);
					cr = null;
					increaseClanLevel = true;
				}
