...
import interlude.gameserver.datatables.EventDroplist;
import interlude.gameserver.datatables.ItemTable;
import interlude.gameserver.datatables.EventDroplist.DateDrop;
...
import interlude.gameserver.instancemanager.PcCafePointsManager;
...
import interlude.gameserver.model.actor.knownlist.AttackableKnownList;
import interlude.gameserver.model.base.SoulCrystal;
 ...
 * L2Attackable :<BR>
 * <BR>
 * <li>L2ArtefactInstance</li> <li>L2FriendlyMobInstance</li> <li>L2MonsterInstance</li> <li>L2FortSiegeGuardInstance</li> <li>L2SiegeGuardInstance</li>
 *
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (Config.CHAMPION_ENABLE)
		{
			...
			if (!(this instanceof L2GrandBossInstance) && this instanceof L2MonsterInstance 
				&& !getTemplate().isQuestMonster && Config.CHAMPION_FREQUENCY > 0 
				&& getLevel() >= Config.CHAMPION_MIN_LVL 
				&& getLevel() <= Config.CHAMPION_MAX_LVL)
			{
...
/*        // If aggro is negative, its comming from SEE_SPELL, buffs use constant 150 
	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		int deepBlueDrop = 1;
		if (Config.DEEPBLUE_DROP_RULES && getNpcId() != 12774 && getNpcId() != 12777)
		{
			if (levelModifier > 0)
			{
		...
		{
			dropChance = 1;
			// if (drop.getItemId() == 57 && dropChance >
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (Rnd.get(L2DropData.MAX_CHANCE) < categoryDropChance)
		{
			...
			// Now decide the quantity to drop based on the rates and
			// penalties.
			// To get this value
			// simply divide the modified categoryDropChance by the base
			// category chance. This
			// results in a chance that will dictate the drops amounts: for
			// each
			// amount over 100
			// that it is, it will give another chance to add to the min/max
			// quantities.
			//
			// For example, If the final chance is 120%, then the item
			// should
			// drop between
			// its min and max one time, and then have 20% chance to drop
			// again.
			// If the final
			// chance is 330%, it will similarly give 3 times the min and
			// max,
			// and have a 30%
			// chance to give a 4th time.
			// At least 1 item will be dropped for sure. So the chance will
			// be
			// adjusted to 100%
			// if smaller.
		...
		return null;
		/*
		 * // Applies Drop rates if (drop.getItemId() == 57) dropChance *= Config.RATE_DROP_ADENA; else if (isSweep) dropChance *= Config.RATE_DROP_SPOIL; else dropChance *= Config.RATE_DROP_ITEMS; // Round drop chance dropChance = Math.round(dropChance); // Set our limits for chance of drop if (dropChance < 1) dropChance = 1; // if (drop.getItemId() == 57 && dropChance > L2DropData.MAX_CHANCE)
		 * dropChance = L2DropData.MAX_CHANCE; // If item is adena, dont drop multiple time // Get min and max Item quantity that can be dropped in one time int minCount = drop.getMinDrop(); int maxCount = drop.getMaxDrop(); int itemCount = 0; if (itemCount > 0) return new RewardItem(drop.getItemId(), itemCount); else if (itemCount == 0 && Config.DEBUG)
		 * _log.fine("Roll produced 0 items to drop..."); return null;
		 */
	}
	...
	private int calculateLevelModifierForDrop(L2PcInstance lastAttacker)
	{
		if (Config.DEEPBLUE_DROP_RULES && getNpcId() != 12774 && getNpcId() != 12777)
		{
			...
			// Check to prevent very high level player to nearly kill mob
			// and
			...
			// According to official data (Prima), deep blue mobs are 9 or
			// more
	 ...
	 * <B><U> Actions</U> : </B><BR>
	 * <BR>
	 * <li>Manage drop of Special Events created by GM for a defined period</li> <li>Get all possible drops of this L2Attackable from L2NpcTemplate and add it Quest drops</li> <li>For each possible drops (base + quests), calculate which one must be dropped (random)</li> <li>Get each Item quantity dropped (random)</li> <li>Create this or these L2ItemInstance corresponding to each Item Identifier
	 * dropped</li> <li>If the autoLoot mode is actif and if the L2Character that has killed the L2Attackable is a L2PcInstance, give this or these Item(s) to the L2PcInstance that has killed the L2Attackable</li> <li>If the autoLoot mode isn't actif or if the L2Character that has killed the L2Attackable is not a L2PcInstance, add this or these Item(s) in the world as a visible object at the
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
	{
		...
		if (Config.CHAMPION_ENABLE && isChampion() && player.getLevel() <= getLevel() && Config.CHAMPION_REWARD > 0 && Rnd.get(100) < Config.CHAMPION_REWARD)
		{
		...
		// only L2Monster with <= 1x HP can drop herbs
		{
			boolean _hp = false;
			boolean _mp = false;
			boolean _spec = false;
	 ...
	 * <BR>
	 * <li>Get an Item Identifier (random) from the DateDrop Item table of this Event</li> <li>Get the Item quantity dropped (random)</li> <li>Create this or these L2ItemInstance corresponding to this Item Identifier</li> <li>If the autoLoot mode is actif and if the L2Character that has killed the L2Attackable is a L2PcInstance, give this or these Item(s) to the L2PcInstance that has killed the
	...
	public void doEventDrop(L2Character lastAttacker)
	{
		...
		if (player == null)
		{
			return; // Don't drop anything if the last attacker or ownere
			// isn't
			// L2PcInstance
		}
	 ...
	 *
	 * @param attacker
	 *            The player that last killed this L2Attackable $ Rewrite 06.12.06 - Yesod
	 */
	private void levelSoulCrystals(L2Character attacker)
	{
		...
		if (!isBossMob)
		{
			// Fail if this L2Attackable isn't absorbed or there's no one in
			// its
			// _absorbersList
			...
			// Check if the soul crystal was used when HP of this
			// L2Attackable
		...
		{
			// This is a naive method for selecting a random member. It gets
			// any random party member and
			// then checks if the member has a valid crystal. It does not
			// select the random party member
			// among those who have crystals, only. However, this might
			// actually be correct (same as retail).
		...
		for (L2PcInstance player : players)
		{
					...
					if (id == itemId)
					{
						...
						if (id != SoulCrystal.RED_NEW_CRYSTAL && id != SoulCrystal.GRN_NEW_CYRSTAL && id != SoulCrystal.BLU_NEW_CRYSTAL)
						{
							try
							{
								...
								{
									// Split the name of the crystal into
									// 'name'
									// & 'level'
			...
			// If the crystal level is way too high for this mob, say that
			// we
			// can't increase it
			...
			/*
			 * TODO: Confirm boss chance for crystal level up and for crystal breaking. It is known that bosses with FULL_PARTY crystal level ups have 100% success rate, but this is not the case for the other bosses (one-random or last-hit). While not confirmed, it is most reasonable that crystals leveled up at bosses will never break. Also, the chance to level up is guessed as around 70% if not
			 * higher.
			 */
			int chanceLevelUp = isBossMob ? 70 : SoulCrystal.LEVEL_CHANCE;
			// If succeeds or it is a full party absorb, level up the
			// crystal.
	...
	// class. The other non-pc instances extend either L2NpcInstance or L2MonsterInstance.
	@Override
