...
public class DimensionalRift
{
	...
	// LogFactory.getLog(DimensionalRift.class.getName());
	...
	protected void createTeleporterTimer(final boolean reasonTP)
	{
		...
		if (reasonTP) {
			teleporterTimer.schedule(teleporterTimerTask, calcTimeToNextJump()); // Teleporter
		// task,
		// 8-10
		// minutes
		} else {
			teleporterTimer.schedule(teleporterTimerTask, seconds_5); // incorrect
		// party
		// member
		// invited.
		}
	...
	{
		if (_choosenRoom == -1)
		{ // Do not tp in the same room a second
			// time
			do {
