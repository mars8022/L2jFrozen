...
import interlude.gameserver.instancemanager.AuctionManager;
...
public class ClanHall
{
	...
	public void setOwner(L2Clan clan)
	{
		...
		if (_ownerId > 0 || clan == null) return;
	...
	private void loadFunctions()
	{
		...
		try
		{
			...
			while (rs.next())
				_functions.put(rs.getInt("type"), new ClanHallFunction(rs.getInt("type"), 
						rs.getInt("lvl"), 
						rs.getInt("lease"), 0, 
						rs.getLong("rate"), 
						rs.getLong("endTime")));
		...
		public void run()
		{
			try
			{
				...
				if (ClanTable.getInstance().getClan(getOwnerId()).getWarehouse().getAdena() >= getLease())
				{
					...
					if (Config.DEBUG) _log.warning("deducted " + getLease() + " adena from " + getName() + " owner's cwh for ClanHall _paidUntil" + _paidUntil);
