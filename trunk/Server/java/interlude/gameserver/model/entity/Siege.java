...
import interlude.gameserver.instancemanager.SiegeManager.SiegeSpawn;
...
public class Siege
{
	...
	// castle siege.]
	// id=646 msg=[You do not have the authority to modify the castle
	// defender
	// list.]
	// - id=688 msg=[The clan that owns the castle is automatically
	// registered
	...
	// id=718 msg=[The castle gates cannot be opened and closed during a
	// siege.]
	// - id=295 msg=[$s1's siege was canceled because there were no clans
	// that
	// participated.]
	...
	// id=663 msg=[The siege time has been declared for $s. It is not
	// possible
	// to change the time after a siege time has been declared. Do you want
	// to
	// continue?]
	// id=667 msg=[You are registering on the attacking side of the $s1
	// siege.
	// Do you want to continue?]
	// id=668 msg=[You are registering on the defending side of the $s1
	// siege.
	// Do you want to continue?]
	// id=669 msg=[You are canceling your application to participate in the
	// $s1
	...
	// - id=845 msg=[The deadline to register for the siege of $s1 has
	// passed.]
	...
	public void midVictory()
	{
		if (getIsInProgress()) // Siege still in progress
		{
			if (getCastle().getOwnerId() > 0)
			{
				_siegeGuardManager.removeMercs(); // Remove all merc entry
				// from db
			}
