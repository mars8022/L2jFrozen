...
public class L2DropCategory
{
	...
	private int _categoryChance; // a sum of chances for calculating if
	// an
	// item will be dropped from this category
	private int _categoryBalancedChance; // sum for balancing drop
	// selection
	...
	// this returns the chance for the category to be visited in order to
	// check
	// if
	...
	/**
	 * ONE of the drops in this category is to be dropped now. to see which one will be dropped, weight all items' chances such that their sum of chances equals MAX_CHANCE. since the individual drops have their base chance, we also ought to use the base category chance for the weight. So weight = MAX_CHANCE/basecategoryDropChance. Then get a single random number within this range. The first item
