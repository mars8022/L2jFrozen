...
public class FortSiegeManager
{
	...
	private int _attackerRespawnDelay = 20000; // Time in ms. Changeable in
	// siege.config
	private int _defenderMaxClans = 500; // Max number of clans
	private int _defenderRespawnDelay = 10000; // Time in ms. Changeable in
	// siege.config
	// Siege settings
	...
	private int _controlTowerLosePenalty = 20000; // Time in ms.
	// Changeable in
	// siege.config
	...
	public final boolean checkIsRegistered(L2Clan clan, int fortid)
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT clan_id FROM fortsiege_clans where clan_id=?");
	...
	private final void load()
	{
		try
		{
			InputStream is = new FileInputStream(new File(FORTSIEGE_CONFIGURATION_FILE));
			...
			for (Fort fort : FortManager.getInstance().getForts())
			{
				...
				for (int i = 1; i < 0xFF; i++)
				{
					...
					try
					{
						...
						_controlTowersSpawns.add(new SiegeSpawn(fort.getFortId(), x, y, z, 0, npc_id, hp));
					}
					catch (Exception e)
					{
						_log.warning("Error while loading control tower(s) for " + fort.getName() + " fort.");
					}
				}
				...
				for (int i = 1; i < 0xFF; i++)
				{
					...
					try
					{
						...
						int npc_id = Integer.parseInt(st.nextToken());
						_artefactSpawns.add(new SiegeSpawn(fort.getFortId(), x, y, z, heading, npc_id));
					}
					catch (Exception e)
					{
						_log.warning("Error while loading artefact(s) for " + fort.getName() + " fort.");
					}
				}
				_controlTowerSpawnList.put(fort.getFortId(), _controlTowersSpawns);
				_artefactSpawnList.put(fort.getFortId(), _artefactSpawns);
			}
		}
		catch (Exception e)
		{
			// _initialized = false;
			System.err.println("Error while loading fort siege data.");
			e.printStackTrace();
		}
	}
	...
	public final void onEnterWorld(L2PcInstance activeChar)
	{
		for (FortSiege siege : getSieges())
		{
