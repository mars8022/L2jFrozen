...
public class FortressofTheDeadManager extends ClanHallSiege
{
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			...
			spawnDoor();
			L2NpcTemplate template = NpcTable.getInstance().getTemplate(35629);
	...
	private boolean checkIfCanRegister(L2PcInstance player)
	{
		...
		else
		{
			...
			if (DevastatedCastleManager.getInstance().checkIsRegistered(player.getClan()))
			{
				player.sendPacket(SystemMessageId.ALREADY_REQUESTED_SIEGE_BATTLE);
				return false;
			}
