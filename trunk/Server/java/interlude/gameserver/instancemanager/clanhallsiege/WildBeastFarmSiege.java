...
public class WildBeastFarmSiege extends ClanHallSiege
{
	...
	public void spawnFlags()
	{
		...
		for (String clanName : getRegisteredClans())
		{
			...
			//L2DecoInstance flag = new L2DecoInstance(null, IdFactory.getInstance().getNextId(), template, false, true, clan);
	...
	public void anonce(String text, int type)
	{
		...
		if (type == 1)
		{/*
		 * for(L2PcInstance ptl : _PartyLeaders) { ptl.sendMessage("Because there was a party 
		 * which doesn't meet a condition, an entry was refused.");
		 */
			for (String clanName : getRegisteredClans())
			{
				L2Clan clan = ClanTable.getInstance().getClanByName(clanName);
