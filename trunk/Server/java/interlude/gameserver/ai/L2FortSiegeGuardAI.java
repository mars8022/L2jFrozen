...
{
	// protected static final Logger _log = Logger.getLogger(L2FortSiegeGuardAI.class.getName());
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Update every 1s the _globalAggro counter to come close to 0</li> <li>If the actor is Aggressive and can attack, add all autoAttackable L2Character in its Aggro Range to its _aggroList, chose a target and order to attack it</li> <li>If the actor can't attack, order to it to return to its home location</li>
	 */
	private void thinkActive()
	{
		...
		// Order to the L2FortSiegeGuardInstance to return to its home location because there's no target to attack
	...
	private void attackPrepare()
	{
		...
		if (_attackTarget instanceof L2PcInstance && fsGuard.getFort().getSiege().checkIsDefender(((L2PcInstance) _attackTarget).getClan()))
		{
			// Cancel the target
			fsGuard.stopHating(_attackTarget);
			_actor.setTarget(null);
			setIntention(AI_INTENTION_IDLE, null, null);
			return;
		}
		...
		{
			// Fort Siege guards differ from normal mobs currently:
