...
public class SSQStatus extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		writeC(0xf5);
		writeC(_page);
		writeC(SevenSigns.getInstance().getCurrentPeriod()); // current
		// period?
		int dawnPercent = 0;
		int duskPercent = 0;
		switch (_page)
		{
			case 1:
				...
				writeD(SevenSigns.getInstance().getPlayerStoneContrib(_activevChar)); // Seal
				// Stones
				// Turned-In
				writeD(SevenSigns.getInstance().getPlayerAdenaCollect(_activevChar)); // Ancient
				// Adena
				// to
				// Collect
			...
			case 3:
				// c cc [ccc (cccc)]
				writeC(10); // Minimum limit for winning cabal to retain their
				// seal
