...
public class SummonItems implements IItemHandler
{
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if(activeChar._inEventTvT && TvT._inProgress && !Config.TVT_ALLOW_SUMMON)
			{
		...
		if(activeChar._inEventDM && DM._inProgress && !Config.DM_ALLOW_SUMMON)
		{
			ActionFailed af = ActionFailed.STATIC_PACKET;
			activeChar.sendPacket(af);
			return;
		...
		if(activeChar._inEventCTF && CTF._inProgress && !Config.CTF_ALLOW_SUMMON)
		{
			ActionFailed af = ActionFailed.STATIC_PACKET;
			activeChar.sendPacket(af);
			return;
