 ...
 * </p>
 * <p>
 * Scheduled Tasks can either be sent to a {@link #_generalScheduledThreadPool "general"} or
 * {@link #_effectsScheduledThreadPool "effects"} {@link ScheduledThreadPoolExecutor ScheduledThreadPool}: The "effects"
 * one is used for every effects (skills, hp/mp regen ...) while the "general" one is used for everything else that
 * needs to be scheduled.<br>
 * There also is an {@link #_aiScheduledThreadPool "ai"} {@link ScheduledThreadPoolExecutor ScheduledThreadPool} used
 * for AI Tasks.
 * </p>
 * <p>
 ...
 * or
 * <ul>
 * <li>{@link #scheduleEffectAtFixedRate(Runnable, long, long)(Runnable, long)} : for effects Tasks that needs to be
 * executed periodicaly.</li>
 * <li>{@link #scheduleGeneralAtFixedRate(Runnable, long, long)(Runnable, long)} : for scheduled Tasks that needs to be
 * executed periodicaly.</li>
 * <li>{@link #scheduleAiAtFixedRate(Runnable, long, long)(Runnable, long)} : for AI Tasks that needs to be executed
 * periodicaly</li>
 * </ul>
 * </p>
 * <p>
 ...
 * {@link com.l2jfrozen.gameserver.model.actor.knownlist KnownList} updates, SQL updates/inserts...)?</li>
 * </ul>
 * </p>
 * 
 * @author -Wooden-
...
//	private ScheduledThreadPoolExecutor _daemonScheduledThreadPool;
	...
	{
		_effectsScheduledThreadPool = new ScheduledThreadPoolExecutor(Config.THREAD_P_EFFECTS, new PriorityThreadFactory("EffectsSTPool", Thread.NORM_PRIORITY, false));
		_generalScheduledThreadPool = new ScheduledThreadPoolExecutor(Config.THREAD_P_GENERAL, new PriorityThreadFactory("GerenalSTPool", Thread.NORM_PRIORITY, false));
//		_daemonScheduledThreadPool = new ScheduledThreadPoolExecutor(10, new PriorityThreadFactory("DaemonSTPool", Thread.NORM_PRIORITY, true));

		_ioPacketsThreadPool = new ThreadPoolExecutor(Config.IO_PACKET_THREAD_CORE_SIZE, Integer.MAX_VALUE, 5L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new PriorityThreadFactory("I/O Packet Pool", Thread.NORM_PRIORITY + 1, false));

		_generalPacketsThreadPool = new ThreadPoolExecutor(Config.GENERAL_PACKET_THREAD_CORE_SIZE, Config.GENERAL_PACKET_THREAD_CORE_SIZE + 2, 15L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new PriorityThreadFactory("Normal Packet Pool", Thread.NORM_PRIORITY + 1, false));

		_generalThreadPool = new ThreadPoolExecutor(Config.GENERAL_THREAD_CORE_SIZE, Config.GENERAL_THREAD_CORE_SIZE + 2, 5L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), new PriorityThreadFactory("General Pool", Thread.NORM_PRIORITY, false));

		// will be really used in the next AI implementation.
		_aiThreadPool = new ThreadPoolExecutor(1, Config.AI_MAX_THREAD, 10L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());

		_aiScheduledThreadPool = new ScheduledThreadPoolExecutor(Config.AI_MAX_THREAD, new PriorityThreadFactory("AISTPool", Thread.NORM_PRIORITY, false));
	}
	...
	/**
	 * Р—Р°РїСѓСЃРє РґРµРјРѕРЅР°
	 * 
	 * @param r РєР»Р°СЃСЃ СЃ РёРЅС‚РµСЂС„РµР№СЃРѕРј Runnable
	 * @param initial РІСЂРµРјСЏ РїРѕРІС‚РѕСЂРµРЅРёСЏ
	 * @param delay Р·Р°РґРµСЂР¶РєР°
	 * @return ScheduledFuture
	 */
/*	public ScheduledFuture<?> scheduleDaemon(Runnable r, long initial, long delay)
	{
		try
		{
			if(delay < 0)
			...
			return _daemonScheduledThreadPool.scheduleAtFixedRate(r, initial, delay, TimeUnit.MILLISECONDS);
		}
		catch(RejectedExecutionException e)
		{
			//shutdown, ignore
...
/*				" | -------",
				" + Daemons:",
				" |- ActiveThreads:   " + _daemonScheduledThreadPool.getActiveCount(),
				" |- getCorePoolSize: " + _daemonScheduledThreadPool.getCorePoolSize(),
				" |- MaximumPoolSize: " + _daemonScheduledThreadPool.getMaximumPoolSize(),
				" |- LargestPoolSize: " + _daemonScheduledThreadPool.getLargestPoolSize(),
				" |- PoolSize:        " + _daemonScheduledThreadPool.getPoolSize(),
				" |- CompletedTasks:  " + _daemonScheduledThreadPool.getCompletedTaskCount(),
				" |- QueuedTasks:     " + _daemonScheduledThreadPool.getQueue().size()
		}; */
	}
...
//			_daemonScheduledThreadPool.awaitTermination(1, TimeUnit.SECONDS);
...
//			_daemonScheduledThreadPool.shutdown();
...
//		_daemonScheduledThreadPool.purge();
